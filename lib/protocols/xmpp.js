...
XMPPProtocol.prototype.registerTask = function (namespaces, taskFunc, nigel) {
    ...
    tasks.push(function (from, message) {
        ...
        if (idx === 0) {
            ...
            taskFunc(message.substr(namespaces.join(' ').length).trim(), stream, nigel.startSession(from));
        }
    });
};
