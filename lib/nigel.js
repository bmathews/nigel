    ...
    function registerPlugin(namespace, pluginDescriptor, protocols) {
        ...
        pluginInstance(self, config, function (taskName, aliases, help, func) {
            protocols.forEach(function (protocol) {
                ...
                protocol.registerTask(nsClone, func, self);
            });
...
function NigelAuth(nigel) {
    ...
    this.generateToken = function(key, session, meta) {
        ...
        tokens[newToken] = { key: key, session: session, meta: meta };
        return newToken;
    };
    ...
    this.get = function (key, session) {
        ...
        if (!credentials[key + "_" + session.id]) {
            var token = self.generateToken(key, session);
            result.reject({
                type: "AUTH", //
                message: "Please go to " + "http://localhost:3001/auth?tId=" + token + " to setup credentials for " + key
            });
        } else {
    ...
    this.setFromToken = function (token, value) {
        if (this.validToken(token)) {
            var tokenData = tokens[token];
            if (tokenData) {
                self.set(tokenData.key, tokenData.session, value);
                delete tokens[token];
            }
        }
    };

    this.set = function (key, session, value) {
