...
module.exports = function (nigel) {
    ...
    function buildInfoIfBroke(nigel, params) {
        var oParams = _.extend({}, params);
        delete params.format;
        return buildInfo(nigel, params)
            .then(function(data) {
                var jsonObj = JSON.parse(data);
                if (jsonObj && jsonObj.result === "FAILURE") {
                    if (oParams.format === "plain") {
                        var culpritString = "";
                        for (var i = 0; i < jsonObj.culprits.length; i++) {
                            culpritString += jsonObj.culprits[i].fullName + "\n";
                        }
                        return culpritString;
                    } else {
                        return jsonObj.culprits;
                    }
    ...
    nigel.registerTask("info", "Gets a specific builds information", buildInfo, ["status"]);
    nigel.registerTask("start", "Starts a new build", startBuild, ["run"]);
    nigel.registerTask("broke", "Retrieves info for the last build if it is broke", buildInfoIfBroke);
};
